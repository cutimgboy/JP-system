import { ChevronRight, Crown, ShieldCheck, Zap, Globe, Download, Upload, FileText, Landmark, User, Bell, Smartphone, Lock, Info, TrendingUp, CircleDollarSign } from 'lucide-react';
...
const highlights = [{
  titleKey: "全网最高收益率 +92.00%",
  descKey: "作为行业领先的二元期权平台，为您提供全网极具竞争力92.%收益，让您的决策更具价值。",
  icon: () => <IconWrapper>
  ...
  titleKey: "0交易手续费用",
  descKey: "平台不收取任何交易手续费，无论交易品种与频次，您的成本始终为零。",
  icon: () => <IconWrapper>
        <CircleDollarSign size={24} strokeWidth={2} />
      </IconWrapper>
}, {
  titleKey: "资金安全，全球合规",
  descKey: "您的资金享有高标准托管机制，遵循全球多重监管要求，层层防护，确保交易环境安全可靠。",
  icon: () => <IconWrapper>
        <ShieldCheck size={24} strokeWidth={2} />
      </IconWrapper>
}, {
  titleKey: "0延迟下单，抓住每一秒波动",
  descKey: "依托毫秒级撮合系统与实时数据对接，订单执行近乎无延迟，助您精准捕捉瞬息万变的市场波动。",
  icon: () => <IconWrapper>
        <Zap size={24} strokeWidth={2} />
      </IconWrapper>
}, {
  titleKey: "全球热门资产，一键交易",
  descKey: "汇聚全球热门资产，一屏尽览投资机会。无论黄金、比特币还是纳指，都在你掌控之中。",
  icon: () => <IconWrapper>
        ...
        <div className="px-4 flex flex-col gap-4 mt-2">
          ...
          <div className="bg-[#14141c] rounded-[24px] px-5 py-3.5 relative overflow-hidden border-[0.5px] border-white/5 shadow-md">
            <div className="flex items-center justify-between mb-2.5">
              ...
              <span className="text-[#8a8a93] text-[12px]">{tx("3048723人已获得")}</span>
            </div>
            ...
            <div className="relative h-[76px]">
              <AnimatePresence mode="wait">
                ...
                left: 0,
                right: 0
              }} dragElastic={0.18} onDragEnd={(_, info) => {
                if (info.offset.x > 45) {
                  showPreviousSlide();
                  ...
                  <div className="flex flex-col justify-center flex-1 pr-1">
                    <h3 className="text-[15px] font-bold mb-1 tracking-tight text-white">{tx(highlights[currentSlide].titleKey)}</h3>
                    <p className="text-[#8a8a93] text-[12px] leading-snug line-clamp-2">
                      {tx(highlights[currentSlide].descKey)}
                    </p>
                  </div>
                </motion.div>
              </AnimatePresence>
            ...
            <div className="flex justify-center gap-2 mt-2.5">
              {highlights.map((_, idx) => <button key={idx} type="button" aria-label={`${tx('切换到第')}${idx + 1}${tx('个尊享特权')}`} onClick={() => setCurrentSlide(idx)} className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentSlide ? 'w-5 bg-white shadow-[0_0_8px_rgba(255,255,255,0.4)]' : 'w-1.5 bg-white/20'}`} />)}
            </div>
          </div>
          ...
          <div className="bg-[#14141c] rounded-[20px] p-5 border-[0.5px] border-white/5 shadow-sm flex justify-between items-center">
            ...
            label: tx("我的银行"),
            path: '/my-bank'
          }].map((item, idx) => <div key={idx} onClick={() => navigate(item.path)} className="flex flex-col items-center gap-2 cursor-pointer group">
          ...
          <div className="bg-[#14141c] rounded-[20px] px-4 py-2 border-[0.5px] border-white/5 shadow-sm">
            ...
            path: '/change-language'
          }].map((item, idx) => <div key={idx} onClick={() => navigate(item.path)} className="flex items-center justify-between py-4 cursor-pointer hover:bg-white/[0.02] -mx-4 px-4 transition-colors border-b border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[14px] font-medium text-white/90 relative">
                    {item.label}
                    {item.hasRedDot && <span className="absolute -top-0.5 -right-2 w-1.5 h-1.5 rounded-full bg-[#ef4444]"></span>}
                  </span>
                </div>
            ...
            <div className="flex items-center justify-between border-b border-white/5 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[14px] font-medium text-white/90">{tx("交易色彩设置")}</div>
                  <div className="mt-1 text-[11px] text-[#8a8a93]">
                    {isReversed ? tx("红跌绿涨") : tx("红涨绿跌")}
                  </div>
                </div>
              </div>
              <button type="button" role="switch" aria-checked={isReversed} onClick={() => setIsReversed(!isReversed)} className={`relative flex h-[28px] w-[48px] shrink-0 items-center rounded-full border p-[3px] transition-colors ${isReversed ? 'border-[#10b981]/40 bg-[#10b981]' : 'border-white/10 bg-[#2a2a36]'}`}>
                <span className={`block h-5 w-5 rounded-full bg-white shadow-[0_2px_8px_rgba(0,0,0,0.35)] transition-transform ${isReversed ? 'translate-x-[20px]' : 'translate-x-0'}`} />
              </button>
            </div>
            <div onClick={() => navigate('/about-us')} className="flex items-center justify-between py-4 cursor-pointer hover:bg-white/[0.02] -mx-4 px-4 transition-colors">
              <div className="flex items-center gap-3">
                <Info size={18} className="text-[#8a8a93]" strokeWidth={1.5} />
                <span className="text-[14px] font-medium text-white/90">{tx("关于我们")}</span>
              </div>
              <ChevronRight size={16} className="text-[#8a8a93]" />
            </div>
          </div>
