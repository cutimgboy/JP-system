...
export function DepositDetail() {
  ...
  const fetchDepositDetail = async () => {
    try {
      ...
      console.error(tx("获取入金详情失败:"), error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!deposit) {
    return <div className="flex min-h-screen items-center justify-center bg-[#09090b] px-5 text-center">
        <div className="text-[#8a8a93]">{tx("未找到入金记录")}</div>
      </div>;
  }
  ...
  const bankLabel = `${deposit.userBankName || deposit.systemBankName || tx("银行卡")}${deposit.userAccountNumber ? ` (${deposit.userAccountNumber.slice(-4)})` : ''}`;
  return <div className="min-h-screen bg-[#09090b] text-white">
      <div className="sticky top-0 z-20 flex h-[60px] items-center justify-between border-b border-white/5 bg-[#09090b]/80 px-4 backdrop-blur-md">
        <button onClick={() => goBackOrNavigate(navigate, '/fund-records')} className="-ml-2 flex h-10 w-10 items-center justify-center rounded-full text-white transition-colors hover:bg-white/10">
          <ChevronLeft size={24} />
        </button>
        <h1 className="absolute left-1/2 -translate-x-1/2 text-[18px] font-medium">{tx("入金详情")}</h1>
        <div className="w-10" />
      </div>
      ...
      <div className="px-5 pb-[100px] pt-6">
        ...
        delay: 0.1
          ...
          <h3 className="mb-6 text-[15px] font-bold">{tx("资金流转状态")}</h3>
          <div className="ml-1">
            {steps.map((step, index) => <div key={step.title} className="relative flex gap-4 pb-8 last:pb-0">
                {index !== steps.length - 1 && <div className={`absolute bottom-0 left-[11px] top-6 w-[2px] ${step.status === 'completed' ? 'bg-[#6c48f5]' : 'bg-white/10'}`} />}
        ...
        delay: 0.2
          ...
          <div className="flex items-center justify-between gap-4">
            <span className="shrink-0 text-[14px] text-[#8a8a93]">{tx("订单编号")}</span>
            <div className="flex min-w-0 items-center gap-2">
              <span className="min-w-0 truncate font-mono text-[14px]">{orderNumber}</span>
              <button type="button" onClick={handleCopy} className="shrink-0 rounded p-1 text-[#6c48f5] transition-colors hover:bg-[#6c48f5]/10">
                {copied ? <Check size={14} /> : <span className="text-[12px] font-medium">{tx("复制")}</span>}
              </button>
            </div>
          </div>

          <div className="flex items-center justify-between gap-4">
            <span className="text-[14px] text-[#8a8a93]">{tx("交易方式")}</span>
            <span className="text-[14px]">{tx("银行卡转账")}</span>
          </div>

          <div className="flex items-center justify-between gap-4">
            <span className="shrink-0 text-[14px] text-[#8a8a93]">{tx("入金账户")}</span>
          ...
          <div className="flex items-center justify-between gap-4">
            <span className="shrink-0 text-[14px] text-[#8a8a93]">{tx("发起时间")}</span>
        ...
        delay: 0.3
          ...
          <p className="text-[12px] leading-relaxed text-[#8a8a93]">{tx("若您的资金流转状态异常或长时间未到账，请及时联系在线客服。您的资金受严格的银行存管协议保护。")}</p>
        </motion.div>
      </div>
    </div>;
}
...
function getTimelineSteps(deposit: DepositDetail, formatDate: (dateString?: string | null) => string) {
  ...
  if (deposit.status === 1) {
    steps.push({
      title: tx("系统处理中"),
      desc: deposit.reviewTime ? formatDate(deposit.reviewTime) : tx("处理完成"),
      icon: <Check size={14} />,
      status: 'completed',
      color: 'border-[#6c48f5] bg-[#6c48f5] text-white'
    }, {
    ...
    steps.push({
      title: tx("系统处理中"),
      desc: deposit.reviewTime ? formatDate(deposit.reviewTime) : tx("处理失败"),
      icon: <Check size={14} />,
      status: 'completed',
      color: 'border-[#6c48f5] bg-[#6c48f5] text-white'
    }, {
      title: tx("入金失败"),
      desc: deposit.remark || tx("审核未通过，请联系在线客服"),
