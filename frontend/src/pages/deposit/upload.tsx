  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white">
      ...
      <div className="sticky top-0 z-20 flex h-[60px] items-center justify-between border-b border-white/5 bg-[#09090b]/90 px-4 backdrop-blur-md">
        ...
        <h1 className="absolute left-1/2 -translate-x-1/2 text-[18px] font-medium text-white">上传转账凭证</h1>
        <div className="w-10" />
      </div>
      ...
      <div className="px-5 pb-[140px] pt-8">
        ...
        >
          <h2 className="mb-3 text-[18px] font-bold">汇款银行</h2>
          <div className="mb-8 rounded-[20px] border border-white/10 bg-[#14141c]/80 p-5 shadow-sm backdrop-blur-xl">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 pl-[56px] text-[14px] text-[#8a8a93]">
              <span>账户名称：{userBankCard?.accountName || '-'}</span>
              <span className="font-mono">账户号码：{maskAccountNumber(userBankCard?.accountNumber)}</span>
            </div>
          </div>

          <h2 className="mb-3 text-[18px] font-bold">存入资金</h2>
          <div className="relative mb-2">
            ...
            <input
              ...
              placeholder="请输入存入金额"
              className="h-[60px] w-full rounded-[16px] border border-white/10 bg-[#14141c] pl-[92px] pr-4 font-mono text-[18px] text-white shadow-sm outline-none transition-colors placeholder:font-sans placeholder:text-[#8a8a93] focus:border-[#6c48f5]/50"
            />
          </div>
          <p className="mb-10 pl-1 text-[13px] text-[#8a8a93]">因银行监管需求，最低存入金额为50USD=1,250,000VND</p>

          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-[18px] font-bold">上传转账凭证</h2>
            <button type="button" className="text-[14px] text-[#6c48f5] transition-colors hover:text-[#5a3bd9]">
              查看示例
            </button>
          </div>
          ...
          <div className="mb-10 flex flex-wrap gap-4">
            {uploadedImages.map((image, index) => (
              <div
                key={image}
                className="group relative flex h-[100px] w-[100px] items-center justify-center overflow-hidden rounded-[16px] border border-white/10 bg-[#1a1a24] shadow-sm"
              >
                <img src={image} alt={`转账凭证 ${index + 1}`} className="h-full w-full object-cover" />
                <button
                  type="button"
                  onClick={() => removeImage(index)}
                  className="absolute right-1 top-1 z-10 flex h-6 w-6 items-center justify-center rounded-full border border-white/20 bg-black/60 shadow-md transition-colors hover:bg-black/80"
                >
            ...
            {uploadedImages.length < 3 && (
              <label className="flex h-[100px] w-[100px] cursor-pointer items-center justify-center rounded-[16px] border-[1.5px] border-dashed border-white/20 transition-all hover:border-white/40 hover:bg-white/5">
          ...
          <div className="space-y-3 rounded-[20px] border border-white/5 bg-[#1a1a24]/50 p-5 text-[13px] leading-relaxed text-[#8a8a93]">
            <p className="mb-2 font-semibold text-white/90">温馨提示</p>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-[#09090b] via-[#09090b]/95 to-transparent p-5 pt-10">
        <button
          ...
          className="h-[52px] w-full rounded-[16px] border border-white/10 bg-[#2a2a36] text-[16px] font-medium text-white shadow-[0_4px_12px_rgba(0,0,0,0.2)] transition-colors hover:bg-[#3a3a46] disabled:cursor-not-allowed disabled:bg-[#1a1a24] disabled:text-white/30"
        >
          {loading ? '提交中...' : '提交转账凭证'}
        </button>
      </div>
        ...
        {showSubmittedModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
            ...
            >
              ...
              <h3 className="mb-2 text-[22px] font-bold tracking-tight text-white">转账凭证已提交</h3>
              ...
              <div className="mb-8 w-full space-y-3 rounded-[20px] border border-white/5 bg-[#1a1a24]/80 p-5 text-left text-[13px] text-[#8a8a93]">
                <p className="mb-1 font-semibold text-white/90">温馨提示</p>
              ...
              <button
                ...
                className="h-[52px] w-full rounded-[16px] border border-white/10 bg-[#2a2a36] text-[16px] font-medium text-white transition-colors hover:bg-[#3a3a46]"
              >
                知道了
              </button>
            </motion.div>
