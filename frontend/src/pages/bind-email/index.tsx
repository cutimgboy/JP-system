...
export function BindEmail() {
  ...
  const title = hasEmail ? '更换绑定邮箱' : '绑定新邮箱';
  const currentVerifyLabel = hasEmail ? '当前绑定邮箱验证码' : '当前绑定手机号验证码';
  ...
  const canSubmit = useMemo(
    () => emailRegex.test(newEmail) && newEmailCode.length === 6,
    [newEmail, newEmailCode],
  );

  const sendCurrentCode = async () => {
    if (!currentVerifyValue) {
      setMessage({ type: 'error', text: hasEmail ? '当前账号未绑定邮箱' : '当前账号未绑定手机号' });
      return;
    }
  ...
  const sendNewEmailCode = async () => {
    ...
    setMessage(null);
    try {
      await apiClient.post('/auth/send-email', { email: newEmail });
      setMessage({ type: 'success', text: '新邮箱验证码已发送' });
    } catch (error: any) {
      setMessage({ type: 'error', text: extractMessage(error.response?.data, '验证码发送失败') });
    } finally {
      setSendingTarget(null);
  ...
  const handleSubmit = async () => {
    if (!canSubmit) {
      setMessage({ type: 'error', text: '请填写新邮箱和验证码' });
      return;
    }
    ...
    try {
      ...
      await refreshUser();
      setMessage({ type: 'success', text: hasEmail ? '邮箱更换成功' : '邮箱绑定成功' });
  ...
  return (
    <BindShell title={title} onBack={() => (step === 1 ? navigate(-1) : setStep(1))}>
      <div className="mb-8">
        <h2 className="text-[24px] font-bold tracking-tight">{step === 1 ? '验证当前账户' : '输入新邮箱'}</h2>
        <p className="mt-2 text-[13px] leading-6 text-[#8a8a93]">
          {step === 1
            ? hasEmail
              ? '更换前需要先验证当前绑定邮箱。'
              : '绑定邮箱前需要先验证当前已绑定手机号。'
            : '新邮箱验证通过后，将立即完成绑定且无需重新登录。'}
        </p>
      </div>

      {step === 1 ? (
        <div className="space-y-5">
          <ReadonlyTarget icon={hasEmail ? Mail : Smartphone} label={hasEmail ? '当前邮箱' : '当前手机号'} value={currentVerifyValue || '未绑定'} />
          <CodeInput
            label={currentVerifyLabel}
            value={currentCode}
            onChange={hasEmail ? setCurrentEmailCode : setCurrentPhoneCode}
...
function ReadonlyTarget({ icon: Icon, label, value }: { icon: typeof Mail; label: string; value: string }) {
  return (
    <div>
