...
import { Book, ChevronLeft, ChevronRight, Copy, Download, Home, Lock, MonitorSmartphone, MoreVertical, PlusSquare, RefreshCw, Search, Share, Star, TrendingUp } from 'lucide-react';
...
export function InstallApp() {
  ...
  return <div className="min-h-screen bg-[#09090b] text-white">
      <div className="sticky top-0 z-20 flex h-[60px] items-center justify-between border-b border-white/5 bg-[#09090b]/90 px-4 backdrop-blur-md">
        ...
        <h1 className="absolute left-1/2 -translate-x-1/2 text-[18px] font-medium">{tx("安装APP应用")}</h1>
        <div className="w-10" />
      </div>

      <div className="px-5 pb-10 pt-6">
        <div className="relative mb-6 flex rounded-[14px] border border-white/5 bg-[#14141c] p-1">
          <motion.div className="absolute inset-y-1 w-[calc(50%-4px)] rounded-[10px] bg-[#2a2a36] shadow-sm" initial={false} animate={{
          ...
          <button onClick={() => setBrowser('safari')} className={`relative z-10 flex-1 rounded-[10px] py-2.5 text-[14px] font-medium transition-colors ${browser === 'safari' ? 'text-white' : 'text-[#8a8a93] hover:text-white/80'}`}>{tx("Safari浏览器")}</button>
          <button onClick={() => setBrowser('chrome')} className={`relative z-10 flex-1 rounded-[10px] py-2.5 text-[14px] font-medium transition-colors ${browser === 'chrome' ? 'text-white' : 'text-[#8a8a93] hover:text-white/80'}`}>{tx("谷歌浏览器")}</button>
        </div>

        <AnimatePresence mode="wait">
          <motion.div key={browser} initial={{
          opacity: 0,
          y: 10
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
            ...
            {browser === 'safari' ? <>
                <StepCard number={1} title={tx("点击底部栏中间的分享按钮")}>
                ...
                <StepCard number={2} title={tx("点击“添加到主屏幕”")} description={tx("在弹出的菜单中，向上滑动找到并点击该选项")}>
                  <div className="bg-[#1a1a24] p-2">
                    <div className="overflow-hidden rounded-[14px] bg-[#2a2a36]">
                      <MenuRow icon={Copy} label={tx("拷贝")} />
                      <div className="relative flex items-center justify-between bg-[#6c48f5]/15 px-4 py-3.5">
                        ...
                        <span className="ml-1 text-[15px] font-medium text-[#a58dff]">{tx("添加到主屏幕")}</span>
                        <PlusSquare size={20} className="text-[#a58dff]" />
                      </div>
                      <MenuRow icon={Search} label={tx("在网页中查找")} />
                    </div>
                  </div>
                </StepCard>
              </> : <>
                <StepCard number={1} title={tx("点击右上角的菜单按钮")}>
                ...
                <StepCard number={2} title={tx("点击“添加到主屏幕”")} description={tx("在下拉菜单中找到此选项以安装")}>
                  <div className="relative h-[180px] bg-[#1a1a24] p-3">
                    <div className="ml-auto w-[220px] rounded-[16px] border border-white/10 bg-[#2a2a36] py-1.5 shadow-xl">
                      <MenuRow icon={Star} label={tx("添加书签")} />
                      <MenuRow icon={Download} label={tx("下载此网页")} />
                      <div className="relative flex items-center gap-3 bg-[#6c48f5]/15 px-4 py-3">
                        ...
                        <span className="text-[14px] font-medium text-[#a58dff]">{tx("添加到主屏幕")}</span>
                      </div>
                      <MenuRow icon={RefreshCw} label={tx("重新加载")} />
                    </div>
                  </div>
                </StepCard>
              </>}

            <StepCard number={3} title={tx("开始交易")} description={tx("回到手机桌面，点击图标即可快速访问")}>
              <div className="flex justify-center bg-[#1a1a24] py-8">
                <div className="grid max-w-[300px] grid-cols-4 gap-x-6 gap-y-4 px-6">
                  {[1, 2, 3, 4].map(item => <div key={item} className="flex flex-col items-center gap-2">
                      <div className={`h-[56px] w-[56px] rounded-[14px] ${item === 3 ? 'flex items-center justify-center border border-white/10 bg-gradient-to-br from-[#6c48f5] to-[#4b2eb3] shadow-[0_4px_16px_rgba(108,72,245,0.4)]' : 'border border-white/5 bg-[#2a2a36]'}`}>
                        {item === 3 ? <div className="relative">
...
function StepCard({
  ...
  return <div className="rounded-[20px] border border-white/5 bg-[#14141c] p-5">
...
function MenuRow({
  ...
  return <div className="flex items-center gap-3 border-b border-white/5 px-4 py-3.5 last:border-b-0">
