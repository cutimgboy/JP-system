...
export function Promotion() {
  ...
  return <div className="min-h-screen bg-[#09090b] text-white flex flex-col">
      ...
      <div className="fixed top-0 left-0 right-0 h-[60px] flex items-center justify-between px-6 shrink-0 z-20 bg-[#09090b]/80 backdrop-blur-md border-b border-white/5">
        ...
        <h1 className="font-medium text-[16px] tracking-wide">{tx("活动详情")}</h1>
        <div className="w-10" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-[100px] pt-[60px]">
        ...
        <div className="relative pt-6 pb-10 px-6 overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col gap-2">
            <span className="text-[#6c48f5] font-semibold text-[13px] tracking-wider uppercase flex items-center gap-1.5 mb-1">
              <Gift size={14} />{tx("新老用户专享")}</span>
            <h2 className="text-[34px] font-bold leading-[1.2] tracking-tight">{tx("入金送500%现金")}<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#10b981] to-[#34d399]">{tx("奖励到账即可提现")}</span>
            </h2>
            <p className="text-[#8a8a93] text-[13px] mt-2 leading-relaxed max-w-[280px]">{tx("参与入金活动，最高可获得 500% 现金返还，交易即可解锁，随时提现。")}</p>
          </div>
        </div>
        ...
        <div className="px-6 mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-[18px] font-bold">{tx("入金奖励梯度")}</h3>
          </div>
          ...
          <div className="mt-4 bg-[#1a1a24] rounded-xl px-4 py-3 flex items-start gap-3 border border-white/5">
            ...
            <p className="text-[12px] text-[#8a8a93] leading-[1.5]">
              <strong className="text-white font-medium">{tx("入金越多奖励越多，上不封顶。")}</strong>{tx("充值金额可累计，达到对应梯度后系统自动发放奖励至您的账户中。")}</p>
          </div>
        </div>
        ...
        <div className="px-6 mb-10">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-[18px] font-bold">{tx("本周入金排行")}</h3>
          </div>
          ...
          <div className="bg-[#14141c] border border-white/5 rounded-[24px] overflow-hidden">
            <div className="grid grid-cols-[1.2fr_2fr_3fr_2fr] gap-2 px-4 py-3 bg-white/[0.02] border-b border-white/5">
              <span className="text-[#8a8a93] text-[11px] text-center">{tx("排名")}</span>
              <span className="text-[#8a8a93] text-[11px]">{tx("用户")}</span>
              <span className="text-[#8a8a93] text-[11px] text-right">{tx("入金金额")}</span>
              <span className="text-[#8a8a93] text-[11px] text-right">{tx("已获奖励")}</span>
            </div>
        ...
        <div className="px-6 mb-10">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-[18px] font-bold">{tx("平台核心优势")}</h3>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            icon: Globe,
            color: "#ec4899",
            title: tx("全球热门资产，一键交易"),
            desc: tx("汇聚全球热门资产，一屏尽览投资机会。无论黄金、比特币还是纳指，都在你掌控之中。")
          }].map((adv, idx) => {
            const Icon = adv.icon;
            return <div key={idx} className="bg-[#14141c] p-4 rounded-[20px] flex gap-4 border border-white/5">
                  <div className="w-10 h-10 shrink-0 rounded-[12px] flex items-center justify-center relative overflow-hidden" style={{
      ...
      <div className="fixed bottom-0 left-0 w-full p-6 pt-4 bg-gradient-to-t from-[#09090b] via-[#09090b]/90 to-transparent z-30">
        <button onClick={() => navigate('/deposit')} className="w-full h-[54px] bg-[#6c48f5] hover:bg-[#5a3ae0] transition-colors rounded-[20px] font-semibold text-[16px] text-white shadow-[0px_8px_20px_rgba(108,72,245,0.3)] flex items-center justify-center gap-2">{tx("立即入金参与")}<ChevronLeft size={18} className="rotate-180" />
        </button>
      </div>
...
function RecentDeposits() {
  ...
  return <div className="px-6 mb-10">
      <div className="bg-[#14141c] border border-white/5 rounded-[20px] p-4">
        <h3 className="text-[14px] font-semibold mb-3 flex items-center gap-2 text-white/90">
          <div className="w-2 h-2 rounded-full bg-[#10b981] animate-pulse" />{tx("实时入金动态")}</h3>
