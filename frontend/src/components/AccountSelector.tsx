...
export function AccountSelector({
  ...
  const fetchBalances = async () => {
    try {
      ...
      console.error(tx("获取账户余额失败:"), error);
    }
  };
  useEffect(() => {
    fetchBalances();
  ...
  return <div className="relative z-50 flex-1" ref={dropdownRef}>
      {/* Header trigger */}
      <div className="flex items-center gap-3 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        ...
        <div className="flex flex-col justify-center gap-0.5">
          <div className="flex items-center gap-1.5">
            <span className="text-[#8a8a93] text-[12px] font-medium tracking-wide">{tx('可用资金')}</span>
