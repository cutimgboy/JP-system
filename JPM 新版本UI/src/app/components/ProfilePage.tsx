...
import bannerImg from 'figma:asset/aa504d021d0ab401e5223db475649354af7666ff.png';
...
const highlights = [
  {
    title: "全网最高收益率 +92.00%",
    desc: "作为行业领先的二��期权���台，为您提供全网极具竞争力92.%收益，让您的决策更具价值。",
    icon: () => (
      <IconWrapper>
  ...
  {
    title: "全球热门资产，一键交易",
    desc: "汇聚全球热门资产，一屏尽览投资机会。无论黄金、比特币还是纳���，都在你掌控之中。",
    icon: () => (
      <IconWrapper>
...
export function ProfilePage() {
  ...
  return (
    <div className="absolute inset-0 bg-[#09090b] flex flex-col overflow-hidden text-white z-0 rounded-[36px]">
      ...
      <div className="flex-1 overflow-y-auto pb-[120px]" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        <style>{`
          .hide-scroll::-webkit-scrollbar { display: none; }
        `}</style>
        
        {/* Top Header Module */}
        <div className="pt-[24px] px-6 pb-4 flex justify-between items-center bg-[#09090b] sticky top-0 z-30">
          <AccountSelector />
          <DepositButton />
        </div>
        ...
        <div className="px-4 flex flex-col gap-4 mt-2">
          ...
          <div className="bg-[#14141c] rounded-[20px] p-5 border-[0.5px] border-white/5 shadow-sm flex justify-between items-center">
            {[
              { icon: Download, label: '存入资金' },
              { icon: Upload, label: '提取资金' },
              { icon: FileText, label: '资金记录' },
              { icon: Landmark, label: '我的银行' },
            ].map((item, idx) => (
              <div 
                key={idx} 
                onClick={() => {
                  if (item.label === '存入资金') {
                    navigate('/deposit');
                  } else if (item.label === '提取资金') {
                    navigate('/withdraw/select-bank');
                  } else if (item.label === '我的银行') {
                    navigate('/my-banks');
                  } else if (item.label === '资金记录') {
                    navigate('/funds-record');
                  }
                }}
          ...
          <div className="relative h-[86px] rounded-[20px] overflow-hidden flex items-center justify-center border border-white/5 shadow-md cursor-pointer group">
            <img 
              src={bannerImg} 
              alt="入金交易，送5倍现金奖励" 
          ...
          <div className="bg-[#14141c] rounded-[20px] px-4 py-2 border-[0.5px] border-white/5 shadow-sm">
            {[
              { icon: User, label: '个人资料', path: '/profile/info' },
              { icon: Bell, label: '消息中心', hasRedDot: true, path: '' },
              { icon: Smartphone, label: '安装APP', path: '/profile/install' },
              { icon: Lock, label: '修改密码', path: '/profile/password' },
              { icon: Globe, label: '切换语言', path: '/profile/language' },
