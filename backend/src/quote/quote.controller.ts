...
export class QuoteController {
   ...
   */
  @Get('symbols')
  @ApiOperation({ summary: '获取订阅的股票列表' })
  @ApiResponse({
    status: 200,
  ...
  async getStockSymbols() {
    try {
      const symbols = await this.quoteService.getSubscribedSymbols();
      return {
        symbols,
        timestamp: new Date().toISOString(),
      };
    } catch (error) {
      throw new HttpException('获取股票列表失败', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
   ...
   */
  @Sse('stream/:code')
  @ApiOperation({ summary: 'SSE 实时行情推送' })
  @ApiParam({
    name: 'code',
    description: '股票代码，如: AAPL.US'
  })
  ...
  streamQuote(@Param('code') code: string): Observable<MessageEvent> {
    return this.quoteService.streamQuote(code);
  }
}